...
public class FakePlayerCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender s, Command c, String l, String[] a) {
        ...
        switch(a[0].toLowerCase()) {
            case "create" -> { if(a.length>1) { m.createFakePlayer(a[1], p.getLocation()); p.sendMessage("§a创建成功"); } }
            case "remove" -> { if(a.length>1) { m.removeFakePlayer(a[1]); p.sendMessage("§c删除成功"); } }
